...
const OAUTH2_CONFIG = {
    ...
    clientID: process.env.OAUTH2_CLIENT_ID || 'payments-service-client',
    clientSecret: process.env.OAUTH2_CLIENT_SECRET || 'payments-service-secret',
    callbackURL: process.env.OAUTH2_CALLBACK_URL || 'http://localhost:3001/auth/callback',
...
const generateToken = (user) => {
    return jwt.sign(
        { 
            sub: user.id, 
            email: user.email,
            scope: user.scope || 'read write payments'
        }, 
        JWT_SECRET,
        { 
